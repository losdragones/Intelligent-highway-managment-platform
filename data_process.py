...
# df = df[df['radar_no']==3276856]
    ...
    def process(self, period):
        ...
        df['intTime'] = df['timestamp'].map(pd.Timestamp.timestamp)
        # 这个时间戳→intTime是正确对应的，但是intTime→时间戳会产生8小时误差
    ...
    def normalize(self):
        ...
        dfSmooth = pd.DataFrame(self.df['volSmooth'])
        scaler = MinMaxScaler()
        dfSmoothNorm = scaler.fit_transform(dfSmooth)
        self.df['volSmoothNorm'] = dfSmoothNorm    
    ...
    '''选择不同模式的交通数据进行预测'''
    def traffic_pattern(self, df, tp):
    ...
    def missing_data(self,p,tp):
        ...
        df = self.traffic_pattern(df, tp)
        # print(df)
        for index, row in df[pd.isna(df['vol'])].iterrows():
            minute = index.minute
            hour = index.hour
            df.loc[index,'vol'] = np.mean(df[(df['timestamp'].dt.minute==minute) & (df['timestamp'].dt.hour==hour)]['vol'])    
        df['intTime'] = df['timestamp'].map(pd.Timestamp.timestamp)
        day = 24*60*60
        df['Day sin'] = np.sin(2 * np.pi * (df['intTime'] % day / day)) 
        df['Day cos'] = np.cos(2 * np.pi * (df['intTime'] % day / day))
    ...
    def abnormal_data(self):
        #交通量低于均值-3*sigma，则用历史平均代替
        df = self.df
        for index, row in df.iterrows():
            minute = index.minute
            hour = index.hour
            vol = row['vol']
            l = df[(df['timestamp'].dt.minute==minute) & (df['timestamp'].dt.hour==hour)]['vol'].values
            # l = list(l)
            l = l.tolist()
            l.remove(vol)
            if (vol < np.mean(l)-3*np.std(l,ddof=1)):
                print('异常数据',index)
                df.loc[index,'vol'] = np.mean(l)
        self.df = df
