...
def main():
    configs = json.load(open('config.json', 'r'))#读取配置文件
    if not os.path.exists(configs['model']['save_dir']): os.makedirs(configs['model']['save_dir'])
    period = configs['data']['periods']#参数
    seq_len = configs['data']['sequence_length']
    tp = configs['traffic_pattern']
    m = configs['model']['name']
    
    data_process = DataProcessor(configs['data']['filepath'])#加载数据处理模块 
    data_process.time_process()#时间处理
    data_process.delete_lane([1,2,3,4,5])#删除车道
    data_process.process(period)#重新调整周期，目前是一分钟
